...
from PyQt5.Qt import (Qt,
                      QButtonGroup, QCheckBox, QEventLoop, QFileDialog,
                      QGridLayout, QLabel, QGroupBox, QHBoxLayout, QLineEdit,
                      QMessageBox, QPushButton, QRadioButton, QRegExp,
                      QRegExpValidator, QSlider, QTabWidget, QTextEdit,
                      QVBoxLayout, QWidget)

from electrum.i18n import _
from electrum.gui.qt.util import (Buttons, CancelButton, CloseButton, OkButton,
                                  pyqtSignal, TaskThread, WindowModalDialog,
                                  WWLabel)
...
from .octo import (OctoPlugin,
                   TIM_NEW, TIM_RECOVER, TIM_MNEMONIC,
...
class MatrixDialog(WindowModalDialog):
    ...
    def __init__(self, parent):
        ...
        for y in range(3):
            for x in range(3):
                button = QPushButton('?')
                button.clicked.connect(partial(self.process_key, ord('1') +
                                               y * 3 + x))
...
class Plugin(OctoPlugin, QtPluginBase):
    icon_paired = ":icons/octo.png"
    icon_unpaired = ":icons/octo_unpaired.png"

    def create_handler(self, window):
        return Octo_Handler(window)

    @hook
    def abort_send(self, window):
        """ Called when the abort dialog is displayed prior to broadcasting
            a transaction.
        Args:
    ...
    def close_wallet(self, wallet):
        ...
        Args:
            wallet: the wallet thas was closed.
        """
        print("close wallet")
    ...
    def init_qt(self, gui):
        """ Called after plugin is activated by ticking box,
            or at startup if already active.
        Args:
    ...
    def receive_list_menu(self, menu, address):
        ...
        """
        print("receive list menu")

    @hook
    def receive_menu(self, menu, addrs, wallet):
        if len(addrs) != 1 or type(wallet) is not Standard_Wallet:
            return
            ...
            if type(keystore) == self.keystore_class:
                def show_address(keystore=keystore):
                    keystore.thread.add(partial(self.show_address, wallet,
                                                addrs[0], keystore))
                menu.addAction(_("Show on {} ({})").format(self.device,
                               keystore.label), show_address)
    ...
    def show_text_edit(self, edit):
        ...
        """
        print("show text edit")

    @hook
    def transaction_dialog(self, dialog):
        """ Called when the transaction dialog is displayed
        Args:
            dialog: electrum_gui.qt.transaction_dialog.TxDialog representing
            the transaction
        """
        print("show transaction dialog")
    ...
    def transaction_dialog_update(self, dialog):
        ...
        Args:
            ...
            the transaction
        """
        print("transaction dialog update")
